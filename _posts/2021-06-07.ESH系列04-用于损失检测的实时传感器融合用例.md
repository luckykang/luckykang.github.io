...
可以使用`sudo make down`命令停止服务，然后检查是否配置了Docker proxy，如果有的话，执行下面命令移除配置文件`mv ~/.docker/config.json ~/.docker/config.json.backup`，然后再次运行启动命令`sudo make run-base`

> 2.如果执行`sudo docker logs -f event-reconciler`命令失败怎么办?

这时候我们可以用命令`make down`来停止，然后再次执行`sudo make run-base`来运行Docker Compose。
